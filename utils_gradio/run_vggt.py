...
def run_vggt(data_path, ckpt_path='checkpoints/model.pt', step=1, depth_conf_thresh=2.0):
    ...
    # # img_name_list = glob.glob(data_path + '/*color.png')
    # # img_name_list = glob.glob(data_path + '/*.jpg')
    # img_name_list = glob.glob(data_path + '/*.JPG')
    ...
    if isinstance(data_path, list):
        img_name_list = data_path[::step]
    elif os.path.isdir(data_path):
        img_name_list = glob.glob(data_path + '/*')
        img_name_list = sorted(img_name_list)[::step]
    else:
        raise ValueError(f'The input data_path {data_path} should be either a list which consist of image paths or a directory which contain images.')
    # Load and preprocess example imagess
    # images = load_and_preprocess_images(img_name_list).to(device)
    images = load_and_preprocess_images(img_name_list[:150]).to(device)
    ...
    with torch.no_grad():
        with torch.cuda.amp.autocast(dtype=dtype):
            ...
            # pcd_path = os.path.join(data_path, '../pcd_vggt_downsampled.ply')
            # o3d.io.write_point_cloud(pcd_path, pcd)

            scale  = 1.0
            depth_ = depth * (depth_conf>depth_conf_thresh).float()
            depth_mini_ = depth_[depth_ > 0.0].min().cpu().item()
            if  depth_mini_ < min_depth:
                scale = min_depth / depth_mini_
            assert scale >= 0.9999999
            depth = depth * scale
