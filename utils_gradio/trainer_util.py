...
def resume_model(self, ckpt_name='latest'):
    logger.info('resuming...')
    old_checkpnts_dir = self.checkpoints_path
    if not os.path.exists(os.path.join(old_checkpnts_dir, self.model_subdir, ckpt_name + ".pth")):
        ckpts = os.listdir(os.path.join(old_checkpnts_dir, self.model_subdir))
        ckpts.sort()
        ckpt_name = ckpts[-1].split('.')[0]
    saved_model_state = torch.load(os.path.join(old_checkpnts_dir, self.model_subdir, ckpt_name + ".pth"))
    logger.info(f'loading model from {os.path.join(old_checkpnts_dir, self.model_subdir, ckpt_name + ".pth")}')
